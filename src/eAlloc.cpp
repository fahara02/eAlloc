...
eAlloc::eAlloc(void *mem, size_t bytes) {
  ...
  if (!add_pool(mem, bytes)) {
    LOG::ERROR(
        "E_ALLOC",
        "Failed to initialize allocator with initial pool (%p, %zu bytes).\n",
        mem, bytes);
  }
  initialised = true;
}
...
void *eAlloc::add_pool(void *mem, size_t bytes) {
  ...
  if ((reinterpret_cast<ptrdiff_t>(mem) % align_size) != 0) {
    LOG::ERROR("E_ALLOC", "add_pool: Memory must be aligned by %zu bytes",
               align_size);
    return nullptr;
  }
  ...
  if (pool_bytes < min_block_size || pool_bytes > max_block_size) {
    LOG::ERROR("E_ALLOC",
               "add_pool: Memory size must be between %zu and %zu bytes.\n",
  ...
  BlockHeader *block = tlsf::offset_to_block_nc(
      mem, -static_cast<tlsf::tlsfptr_t>(alloc_overhead));
...
void eAlloc::remove_pool(void *pool) {
  for (size_t i = 0; i < pool_count; ++i) {
    if (memory_pools[i] == pool) {
      BlockHeader *block = tlsf::offset_to_block_nc(
          pool, -static_cast<int>(tlsf::alloc_overhead()));
      BlockHeader *next = tlsf::next(block);
      if (!tlsf::is_free(block) || tlsf::get_size(block) != pool_sizes[i] ||
          !tlsf::is_last(next)) {
        LOG::ERROR("E_ALLOC",
                   "Cannot remove pool %p: it contains allocated blocks.\n",
                   pool);
        return;
      }
      int fl = 0, sl = 0;
...
void eAlloc::walk_pool(void *pool, Walker walker, void *user) {
  ...
  while (block && !tlsf::is_last(block)) {
    pool_walker(tlsf::to_ptr_nc(block), tlsf::get_size(block),
                !tlsf::is_free(block), user);
...
void *eAlloc::memalign(size_t align, size_t size) {
  ...
  const size_t size_with_gap =
      tlsf::adjust_request_size(adjust + align + gap_minimum, align);
  const size_t aligned_size =
      (adjust && align > tlsf::align_size()) ? size_with_gap : adjust;
  BlockHeader *block = tlsf::locate_free(&control, aligned_size);
  if (!block)
    return nullptr;
  dsa_assert(sizeof(BlockHeader) ==
             tlsf::min_block_size() + tlsf::alloc_overhead());
...
void *eAlloc::realloc(void *ptr, size_t size) {
  ...
  if (ptr && size == 0) {
    ...
    const size_t combined =
        cursize + tlsf::get_size(next) + tlsf::alloc_overhead();
    const size_t adjust = tlsf::adjust_request_size(size, tlsf::align_size());
...
eAlloc::StorageReport eAlloc::report() const {
  ...
  for (size_t fl = 0; fl < tlsf::cabinets(); ++fl) {
    if (!(control.fl_bitmap & (1U << fl)))
      continue;
    for (size_t sl = 0; sl < tlsf::shelves(); ++sl) {
      if (!(control.cabinets[fl].sl_bitmap & (1U << sl)))
        continue;
      ...
      while (current != &control.block_null) {
        ...
        if (size > report.largestFreeRegion)
          report.largestFreeRegion = size;
  ...
  if (report.totalFreeSpace > 0) {
    report.fragmentationFactor =
        1.0 -
        static_cast<double>(report.largestFreeRegion) / report.totalFreeSpace;
  } else {
